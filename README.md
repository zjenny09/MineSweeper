...
- 天空第17关起加入概率触发的随机助益：随机安全格正常展开，随机污染核心自动标记
- 天空第19关起加入每局最多成功一次的70%概率云盾保护
- 第21关加入三道雷眼Boss机制，正确标记邻近污染并双击数字格即可击破
- 后续雷眼按球面图距离迁移到远处新区域，避免连续出现在同一片已展开区域
- 随机助益、云盾与雷眼均加入独立演出、目标转向和结果反馈

### v0.5.0 更新

- 完成21关三章节主流程：第1–7关陆地篇、第8–14关海洋篇、第15–21关天空篇
- 三章节地图、进度存档、共享扫描能量、Boss通关翻页与章节往返流程正式接通
- 海洋篇加入珊瑚墙六邻接阻断、洋流提示与潮汐分区玩法
- 天空篇加入只由五边形和六边形组成的递增球面棋盘，以及旋转、标记、扫描和双击数字展开
- 第15–21关依次接入云冠、风环、虹桥、浮岛、云脊、雷眼和天宫生态素材及失败状态
- Windows导出版改为从Godot导入资源加载角色与生态动画，并显式包含球面结构数据
...
- `--level=1` 至 `--level=21` 可绕过菜单直接测试，且不写入关卡进度
